...
import Swiper, { Parallax, Navigation, Controller, EffectFade } from 'swiper';
...
const gallery = new Swiper('.gallery-swiper', {
  modules: [Parallax, Controller, Navigation, EffectFade],
  parallax: true,
  lazy: true,
  slidesPerView: 1,
  speed: 1000,
  ...
  on: {
    init: function() {
      // Инициализация счетчика при создании слайдера
      updateCounter(this);
    },
...
function updateCounter(swiper) {
  ...
  if (currentElement && totalElement) {
    ...
    gsap
      ...
      .add(() => {
        currentElement.textContent = visibleIndex + 1;
      }, '<')
      .fromTo(
        currentElement,
        {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Инициализируем первую категорию при загрузке
...
if (window.innerWidth > 767) {
  sideSwitchArrow(
    gallery,
    document.querySelector('.gallery-arrow'),
